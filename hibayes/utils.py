...
def histogramMode(datafile, index=0, nbins=1000, bins=None):
    ...
    if index == 0:
        binmin = 0.0; binmax = 100.0
    elif index == 1:
        binmin = -2.5; binmax = -0.1
    elif index == 2:
        binmin = 1.0; binmax = 500.0
    elif index == 3:
        binmin = 500.0; binmax = 1000.0
...
def reportRelativeEvidences(globList):
    ...
    for run in globList:
        ...
        Z = ana['global evidence'];
        dZ = ana['global evidence error']
        Hdict[Z] = run;
        Zdict[run] = Z;
        dZdict[run] = dZ
...
def reportRelativeEvidence(H0=None, H1=None, verbose=True):
    ...
    Z0 = ana0['global evidence'];
    dZ0 = ana0['global evidence error']
    Z1 = ana1['global evidence'];
    dZ1 = ana1['global evidence error']
